...
class CustomAutoSection extends StatelessWidget {
  final ScrollController scrollController;

  const CustomAutoSection({super.key, required this.scrollController});
  ...
  void _handleStateListener(
      BuildContext context, CustomAutoSessionState state) {
    if (state.isError) {
  ...
  Widget _buildStateContent(
    ...
    if (state.isLoading) {
      ...
      return const Center(
          child: EmptyStateWidget(message: "No Custom Sessions Yet"));
    } else {
  ...
  Widget _buildLoadingList(BuildContext context) {
    return Skeletonizer(
      ...
      child: ListView.separated(
        key: const ValueKey("custom-auto-list-loading"),
        itemCount: 2,
