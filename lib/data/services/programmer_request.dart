...
class ProgrammerRequestService {
  ...
  Future<Either<Failure, ProgrammingRequest>> applyForProgrammer() async {
    final response =
        await baseApiProvider.get<Map<String, dynamic>>(endpoint: '/programer');

    if (response.isSuccess) {
      return Right(ProgrammingRequest.fromJson(response.data!));
    } else {
      return Left(ServerFailure(message: response.error!.message));
    }
  }

  Future<Either<Failure, ProgrammingRequest>> checkForRequest() async {
    final response = await baseApiProvider.get<Map<String, dynamic>>(
        endpoint: '/programer/programmer-request');
